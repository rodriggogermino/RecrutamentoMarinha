<?php
    ...
    $id_concurso = isset($_GET['id']) ? (int)$_GET['id'] : 0;


    $sqlDetalhe = "SELECT * FROM concursos WHERE id = :id";
    $stmtDetalhe = $pdo->prepare($sqlDetalhe);
    $stmtDetalhe->bindParam(':id', $id_concurso, PDO::PARAM_INT);
    $stmtDetalhe->execute();
    $concursoAtual = $stmtDetalhe->fetch(PDO::FETCH_ASSOC);
            ...
            <div id="concursosInfoSectionContainer">
                ...
                <div id="InfoConcurso">
                    <div id="tituloInfoConcurso">
                        <p><?= htmlspecialchars($concursoAtual['numero_concurso']) ?></p>
                        <h1><?= htmlspecialchars($concursoAtual['categoria']) ?> - <?= htmlspecialchars($concursoAtual['titulo']) ?></h1>
                    </div>
                    
                    <p><b>Tipo de Contrato:</b> <?= htmlspecialchars($concursoAtual['tipo_contrato']) ?></p>
                    
                    <div id="datasInfoConcurso">
                        <p><b>Inicio:</b> <?= date('d/m/Y', strtotime($concursoAtual['data_inicio'])) ?></p>
                        <p><b>Fim:</b> <?= date('d/m/Y', strtotime($concursoAtual['data_fim'])) ?></p>
                        <?php if(!empty($concursoAtual['data_incorporacao'])): ?>
                            <p><b>Incorporação:</b> <?= date('d/m/Y', strtotime($concursoAtual['data_incorporacao'])) ?></p>
                        <?php endif; ?>
                    </div>
                ...
                <div id="outrosConcursos">
                    ...
                    <div id="listaConcursosInfo">
                        <?php
                        $sqlOutros = "SELECT * FROM concursos WHERE estado = 'aberto' AND id != :id ORDER BY data_fim ASC LIMIT 3";
                        $stmtOutros = $pdo->prepare($sqlOutros);
                        $stmtOutros->bindParam(':id', $id_concurso, PDO::PARAM_INT);
                        $stmtOutros->execute();
                        $outrosConcursos = $stmtOutros->fetchAll(PDO::FETCH_ASSOC);

                        foreach ($outrosConcursos as $outro):
                        ?>
                            <div id="abertos" class="cardConcurso"> 
                            <p><?= htmlspecialchars($outro['numero_concurso']) ?></p>
                                <h2><?= htmlspecialchars($outro['categoria']) ?> - <?= htmlspecialchars($outro['titulo']) ?></h2>
                                <p><?= htmlspecialchars($outro['descricao']) ?></p>
                               
                                
                                <div class="btnsConcursos">
                                    <?php if(!empty($outro['diario_republica_url'])): ?>
                                        <a href="<?= htmlspecialchars($outro['diario_republica_url']) ?>" target="_blank" class="btnAvisoConcurso">Aviso</a>
                                    <?php endif; ?>
                                    
                                    <a href="concursoInfo.php?id=<?= $outro['id'] ?>" class="btnSaberMaisConcurso">Saber Mais</a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
